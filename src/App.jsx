...
export default function App() {
  ...
  const initializePayment = (amount) => {
    const handler = window.PaystackPop.setup({
      ...
      callback: async () => {
        ...
        alert(`Vortex Signal Boosted! ₦${amount} credited.`);
      },
    });
    handler.openIframe();
  };
  ...
  const handleLinkPhone = async () => {
    ...
    await updateDoc(doc(db, "users", user.uid), { 
      ...
      searchIndex: phoneInput.replace(/\s+/g, '') 
    });
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-6">
            <div className="bg-[#11172b] rounded-3xl p-4 flex items-center gap-3 border border-white/5">
              ...
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search signal..." className="bg-transparent outline-none text-xs w-full" />
            </div>
          </div>
            ...
            {users.filter(u => u.displayName.toLowerCase().includes(searchQuery.toLowerCase())).map(u => (
              <div key={u.uid} onClick={() => setSelectedUser(u)} className="flex items-center gap-4 p-4 bg-[#11172b]/80 rounded-[28px] border border-white/5 active:scale-95 transition-all">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">{u.phoneNumber ? `ID: ${u.phoneNumber}` : "SIGNAL ONLY"}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {!selectedUser && activeTab === "market" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 mb-4">
             <div className="bg-[#11172b] p-4 rounded-[30px] border border-white/5 flex gap-2">
                <input value={newIdea} onChange={(e) => setNewIdea(e.target.value)} placeholder="Share a signal idea..." className="bg-transparent flex-1 outline-none text-xs" />
                <button onClick={postIdea} className="bg-green-600 p-3 rounded-2xl"><TrendingUp size={18} /></button>
             </div>
          </div>
          <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-24">
            {marketIdeas.map(idea => (
              <div key={idea.id} className="p-6 bg-gradient-to-br from-[#11172b] to-[#0d1225] rounded-[35px] border border-white/5 shadow-xl">
                <div className="flex items-center gap-3 mb-4">
                  <img src={idea.authorPhoto} className="w-8 h-8 rounded-full" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{idea.authorName}</p>
                </div>
                <p className="text-sm font-medium leading-relaxed">{idea.text}</p>
                <div className="mt-4 flex justify-between items-center">
                   <span className="text-[8px] font-black text-slate-600 uppercase">{formatTime(idea.createdAt)}</span>
                   <button className="flex items-center gap-1 text-[9px] font-black text-green-500 uppercase bg-green-500/10 px-3 py-1.5 rounded-full border border-green-500/20">
                     <DollarSign size={10} /> Tip Signal
                   </button>
                </div>
              </div>
            ))}
      ...
      {!selectedUser && activeTab === "settings" && (
        <div className="flex-1 flex flex-col p-8 overflow-y-auto pb-32">
           <header className="flex flex-col items-center mb-8 text-center">
              ...
              <p className="text-[10px] font-black text-green-500 tracking-widest uppercase mt-1">{userData?.phoneNumber || "NOT LINKED"}</p>
           </header>
           ...
           <div className="bg-gradient-to-br from-[#1a2238] to-[#0d1225] p-6 rounded-[35px] border border-white/10 shadow-2xl mb-8 relative">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Vortex Wallet</p>
              ...
              <button onClick={() => initializePayment(1000)} className="w-full bg-green-600 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest">Deposit Credits +</button>
           </div>
           
           <div className="space-y-4">
              <button onClick={() => setStealthMode(!stealthMode)} className="p-6 w-full rounded-[30px] border border-white/5 bg-[#11172b] flex justify-between items-center group transition-all">
                 ...
                 <div className={`w-12 h-6 rounded-full relative transition-colors ${stealthMode ? 'bg-green-500' : 'bg-slate-800'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${stealthMode ? 'left-7' : 'left-1'}`}></div>
                 </div>
              </button>
        ...
        <div className="fixed inset-0 z-50 bg-[#060a16] flex flex-col">
          <header className="p-4 flex items-center gap-4 border-b border-white/5 bg-[#0d1225]">
            ...
            <div className="flex-1">
              ...
              <p className="text-[8px] font-black text-green-500 uppercase tracking-widest">{selectedUser.typing ? 'Typing Signal...' : 'ENCRYPTED 🔒'}</p>
            </div>
          </header>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {messages.map((m) => (
              <div key={m.id} className={`flex flex-col ${m.senderId === user.uid ? 'items-end' : 'items-start'}`}>
                <div className={`
                   max-w-[80%] px-4 py-3 rounded-[24px] transition-all duration-500
                   ${m.senderId === user.uid ? 'bg-green-600 rounded-tr-none' : 'bg-[#11172b] rounded-tl-none border border-white/5'}
                   ${stealthMode && m.senderId !== user.uid ? 'blur-[8px] select-none hover:blur-0' : ''}
                `}>
                ...
                <div className="flex items-center gap-1 mt-1">
                   ...
                   {m.senderId === user.uid && (m.seen ? <CheckCheck size={10} className="text-green-500" /> : <Check size={10} className="text-slate-500" />)}
                </div>
              </div>
            ))}
            <div ref={scroll}></div>
      ...
      {!selectedUser && (
        <nav className="p-4 px-8 bg-[#0d1225] flex justify-between border-t border-white/5 pb-10">
          ...
          <button onClick={() => setActiveTab("market")} className={`flex flex-col items-center gap-1.5 ${activeTab === 'market' ? 'text-green-500' : 'text-slate-600'}`}>
            <Global size={22} /><span className="text-[8px] font-black uppercase tracking-widest">Market</span>
          </button>
